...
def get_environment_variables():
    """
    필수 환경 변수:
      - SONGDO_DDANGYO_ID (땡겨요 아이디)
      - SONGDO_DDANGYO_PW (땡겨요 비밀번호)
      - SERVICE_ACCOUNT_JSON_BASE64 (Base64 인코딩된 Google Service Account JSON)
    """
    ddangyo_id = os.getenv("SONGDO_DDANGYO_ID")
    ddangyo_pw = os.getenv("SONGDO_DDANGYO_PW")
    service_account_json_b64 = os.getenv("SERVICE_ACCOUNT_JSON_BASE64")

    if not ddangyo_id or not ddangyo_pw:
        raise ValueError("SONGDO_DDANGYO_ID 혹은 SONGDO_DDANGYO_PW 환경변수가 설정되지 않았습니다.")
...
def update_google_sheets(total_order_amount, aggregated_products, service_account_json_b64):
    """
    - "송도 일일/월말 정산서" 스프레드시트의 "송도" 시트에서 U3:U33(날짜)와 Z3:Z33(주문 총액)을 업데이트
